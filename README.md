# A local coding agent powered by Qwen
...
file/shell tools; **llama.cpp** runs the Qwen model that decides what to do next.
Setup downloads the weights, prepares these runtimes, and installs the `qwen`
command on your PATH. Model inference runs on your own machine, or on your
chosen model host over SSH. An optional Codex CLI launcher is also included.
...
The project machine and model host can be the same box. With `qwen --host
...
[OpenCode](#opencode-chatsh) for details. Web search uses a separate hosted Exa
service; it is not part of local model inference.

**An RTX 3090 is the default preset, not a requirement.** The supplied
[`config.toml`](config.toml) tunes Qwen3.8-27B Q4_K_M for that card's 24 GB VRAM.
You can select another NVIDIA architecture, a Vulkan GPU backend, or CPU
operation. See [Hardware and configuration](#hardware-and-configuration).
...
./chat.sh shutdown          # stop Qwen and release its memory
```
...
After running setup, `qwen`, `qwen load`, and `qwen shutdown` work from any directory.
...
for the 16.5 GB model plus the Nix builds, and the drivers for your selected
backend. The default CUDA preset needs an NVIDIA driver (`nvidia-smi` works);
CPU operation does not. The Codex CLI is optional (`nix profile add nixpkgs#codex`).
...
can take 10-20 minutes the first time), downloads
the 16.5 GB model from `unsloth/Qwen3.8-27B-GGUF`, prepares OpenCode locally,
installs the Codex configuration, and adds `qwen` and `codex-qwen` to your user
Nix profile as one `qwen-tools` entry. Both normal setup and `--client` install
these commands on PATH automatically. Nix's shell initialization makes the
profile available; start a new login shell if Nix was just installed.
...
`nix profile remove qwen-tools`. `./chat.sh` also works without installation.
...
Setup also installs the commands on PATH and prepares the optional Codex
configuration.
...
Alternatively, set `QWEN_CONFIG=/absolute/path/to/settings.toml`; that file
...
Weights still occupy 16.5 GB, with additional RAM/VRAM needed for context and
the runtime. CPU operation and partial GPU offload can be substantially slower.
This configuration tunes the bundled Qwen model; it does not choose a smaller
model automatically.
...
If you change the context or output limit, also match the limits in the client
configuration: `provider.qwen-local.models` → `limit` in `opencode.json`, and
`model_context_window` in `codex/qwen.config.toml` if using Codex. With multiple
parallel slots, set the client's context limit to the available per-slot
context. Hardware settings apply on the server; agent limits are client settings.
...
`qwen` command on your PATH. The checkout can live anywhere:

```bash
git clone <this repo> ~/git/qwen
cd ~/git/qwen
./scripts/setup.sh  # prepare runtimes and weights, and install qwen on PATH
qwen load           # optional: load the model now
```

If `qwen` is already on your PATH, no extra installation is needed. To install
...
the first client connects, run `qwen load` on the model host.
...
```bash
~/git/qwen/chat.sh --host qwen-gpu
```
...
Verify `ssh qwen-gpu true`, then use `--host qwen-gpu` below.
...
```bash
git clone <this repo> ~/git/qwen
~/git/qwen/scripts/setup.sh --client  # install the client and qwen on PATH
cd ~/git/my-project
qwen --host user@gpu-box
qwen --host user@gpu-box run "Fix the failing tests"
qwen --host user@gpu-box /path/to/another/project
```
...
```bash
~/git/qwen/chat.sh --host gpu-box load
~/git/qwen/chat.sh --host gpu-box shutdown  # stops the shared model for everyone
~/git/qwen/chat.sh --host gpu-box --remote-dir /srv/qwen
```

SSH aliases from `~/.ssh/config` work, including custom users, ports, keys,
and jump hosts. By default, the launcher finds the executable `qwen` on the
...
```bash
ssh qwen-gpu 'bash -lc "command -v qwen"'
```

If you prefer to use an uninstalled checkout, `--remote-dir /srv/qwen` (or
`QWEN_REMOTE_DIR`) explicitly selects its `chat.sh` instead. Relative paths
...
```bash
export QWEN_HOST=user@gpu-box
qwen
```

Unset `QWEN_HOST` to use a local model again. `PORT` selects the model host's
API port (default 8080); the SSH port comes from your SSH configuration.
`--local-port` or `QWEN_LOCAL_PORT` selects the
...
The SSH launcher is implemented for OpenCode (`chat.sh` / `qwen`); the optional
`codex-qwen` launcher still uses a local model.

### Codex CLI: `scripts/codex-qwen.sh`

The Codex CLI pointed at the local model, with its Linux sandbox (Landlock and
seccomp) and approval prompts. Run it from the project you want to work on:

```bash
cd ~/git/my-project
~/git/qwen/scripts/codex-qwen.sh
~/git/qwen/scripts/codex-qwen.sh exec "fix the failing test and rerun it"
```

The `qwen` profile defaults to `workspace-write`: the model can read
everything, write only inside the current project, and has no network. Anything
outside that asks for approval. Overrides per run:

| Flag | Effect |
|---|---|
| `-s read-only` | no writes at all |
| `-s workspace-write` | writes inside the project only (default) |
| `-s danger-full-access` | no sandbox |
| `-a untrusted` | approve every command |
| `-a on-request` | model asks when it needs more access (default) |
| `-a never` | never ask; blocked actions just fail |
| `--add-dir DIR` | make another directory writable |
| `--approve-for-me` | auto-review approval requests |
| `--dangerously-bypass-approvals-and-sandbox` | disable everything (use inside an outer sandbox) |

Two sandbox effects specific to this machine: the sandbox blocks the Nix daemon
socket, so `nix-shell` and `nix run` fail inside it, and it hides `/dev/nvidia*`,
so `nvidia-smi` and CUDA programs fail inside it. In both cases the model will
ask for approval to run outside the sandbox, or you can start with
`-s danger-full-access` when the task needs it.

Codex only runs in Git repositories or directories you have marked trusted
(`codex exec` also accepts `--skip-git-repo-check`). The profile lives in
[`codex/qwen.config.toml`](codex/qwen.config.toml); edit it there and rerun
`scripts/setup.sh` to reinstall it.

### OpenCode: `chat.sh`

OpenCode with the same model, configured by [`opencode.json`](opencode.json).
Unlike the Codex path it has no OS sandbox, and the shipped configuration allows
file edits, shell commands, and access outside the project without prompting.
It runs with your normal user permissions.

```bash
cd ~/git/my-project
~/git/qwen/chat.sh                             # full-screen UI
~/git/qwen/chat.sh -c                          # resume the last session
~/git/qwen/chat.sh run "Explain this project"  # one task, no UI
~/git/qwen/chat.sh session list
```
...
Sampling recommended by the model card: instruct mode `temperature 0.7,
top_p 0.8, top_k 20, presence_penalty 1.5`; thinking mode `temperature 1.0,
top_p 0.95`.
...
Both chat launchers start it automatically as a transient user service
named `qwen-model` and leave it loaded so the next session starts instantly.
...
systemctl --user status qwen-model
journalctl --user -u qwen-model -n 40
...
default from `server.reasoning`), `PORT` (default 8080), and `QWEN_CONFIG`
...
| File | Purpose |
|---|---|
| `chat.sh` | start chat, load the model, or shut it down; the only root script |
...
| `scripts/shutdown.sh` | stop Qwen and release its GPU memory |
| `scripts/codex-qwen.sh` | optional Codex launcher |
| `codex/provider.toml`, `codex/qwen.config.toml` | Codex provider block and profile |
...
TOML configuration, runtime flags, SSH argument handling, working-directory preservation, and tunnel cleanup
using isolated mock commands. These checks do not connect to an SSH host.

Reference: [Qwen 3.8 27B hardware guide](https://www.contextstudios.ai/blog/qwen-3-8-27b-hardware-guide),
